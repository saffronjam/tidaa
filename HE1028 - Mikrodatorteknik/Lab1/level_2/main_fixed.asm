...
STOPF	equ	0x003
	...
	org	0x00020
MAIN	RCALL	MINIT
LMLOOP0	BTFSS	PORTB,RB0   ;Waits for switch to be pulled
	BRA	LMLOOP0
LMLOOP1	BTFSC	PORTB,RB0   ;Waits for switch to be pulled back
	BRA	LMLOOP1
	CLRF	COUNTER
LMCOUNT	RCALL	ED250MS	    ;Wait 0.25 seconds
	...
	BSF	STOPF,0	    ;Set stop-flag
	BTFSS	PORTB,RB5   ;Check if switch is pulled back
	BTFSS	STOPF,0	    ;and if it was previously set
	BRA	LMCOUNT	    
	BCF	STOPF,0	    ;If PC is here, we have interupted the counter
	BRA	INCBOT	    ;Increase bot

INCTOP	BTFSS	STOPF,0	    ;Before increasing top, check if switch is cleared
	BRA	LMLOOP2
	BRA	LMLOOP3
LMLOOP2	BTFSS	PORTB,RB5   ;Waits for switch to be pulled
	BRA	LMLOOP2
LMLOOP3	BTFSC	PORTB,RB5   ;Waits for switch to be pulled back
	BRA	LMLOOP3	
	RLNCF	CTOP
	RCALL	ECUL
	CLRF	STOPF
