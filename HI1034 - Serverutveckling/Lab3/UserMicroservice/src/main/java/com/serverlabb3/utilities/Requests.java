...
public class Requests {
    ...
    public static void execute(Request request) throws BackendException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                throw new BackendException("Failed to get response: " + response.message(), HttpStatus.BAD_REQUEST);
            }
        } catch (IOException e) {
            e.printStackTrace();
            throw new BackendException("Couldn't parse response: " + e.getMessage(), HttpStatus.CONFLICT);
        }
    }
