...
public class Controller {
    ...
    /**
     * Queries the database to fetch every review for a specific book
     * @param book
     * @return A list of every review for a book. This can be empty.
     */
    public List<Review> getAllReviewsForBook(Book book)
    {
        List<Review> result = null;
        try
        {
            result = booksDb.getReviewsByBook(book);
        }
        catch (IllegalStateException ise)
        {
            booksView.showAlertAndWait("Failed to get reviews for book\nReason: " + ise.getMessage(), ERROR);
        }
        catch(SQLException sqle){
            booksView.showAlertAndWait("Failed to get reviews for book from database!\nReason: " + sqle.getMessage() + "\n\nPlease try again", ERROR);
        }
        return result;
    }
    ...
    protected void onRateSelectedBook(String description, int rating)
    {
        ...
        Thread thread = new Thread(() -> {
            try {
                var convertedDate = java.sql.Date.valueOf(LocalDate.now());
    ...
    protected void onConnect()
    {
        try {
            ...
            booksView.showAlertAndWait("Failed to connect to database!\nReason: " + sqle.getMessage() + "\n\nPlease try again",ERROR);
        }
        catch (IOException ioe) {
            booksView.showAlertAndWait("Database error. " + ioe.getMessage(),ERROR);
        }
    }
    ...
    protected void onDisconnect()
    {
        try {
            ...
            booksView.showAlertAndWait("Failed to disconnect from database!\nReason: " + sqle.getMessage() + "\n\nPlease try again",ERROR);
        }
        catch (IOException ioe) {
            booksView.showAlertAndWait("Database error. " + ioe.getMessage(),ERROR);
        }
