...
import com.serverlabb3.utilities.forms.*;
...
public class UserService {
    ...
    public UserService(UserRepository userRepo, Ip ip) {
    ...
    public AuthUserVm login(String usernameOrPassword, String password) {
        try {
            ...
            if (userOpt.isPresent()) {
                ...
                System.out.println("Notifying " + ip.getVertxMs() + " about user login. Token: " + userOpt.get().getToken());
                Requests.execute(Requests.createPost(ip.getVertxMs() + "/report", body));
