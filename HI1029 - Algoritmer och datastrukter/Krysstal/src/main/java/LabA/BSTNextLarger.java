...
public class BSTNextLarger<E extends Comparable<E>> {
    ...
    public E getNextLarger(E to) {
        if (to == null) throw new IllegalArgumentException("'to' was null");
        Node<E> current = root, highest = null;
        while (current != null) {
            if (current.data.compareTo(to) > 0) {
                highest = current;
                current = current.left;
            } else {
                current = current.right;
        ...
        return highest == null ? null : highest.data;
    }
}
